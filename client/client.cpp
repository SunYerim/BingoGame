...
int main(int argc, char *argv[])
{
	...
	SOCKET sock = socket(AF_INET, SOCK_STREAM, 0);	 	//소켓생성  
	if (sock == -1)
		printf("socket() error");
	...
	for (a = 0; a < 10; a++)					//빙고 시작
	{
		...
		//sleep(1);
		send(sock, cho, sizeof(cho), 0);				//server한테 선택 값 전송
		recv(sock, cho, sizeof(cho), 0);				//server로부터 선택 값 수신
